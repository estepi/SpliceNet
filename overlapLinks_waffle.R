...
allNodes <- unique(unlist(nodesList))
length(allNodes)
...
dev.off()
#######################################
allLinks <- unique(unlist(linksList))
length(allLinks)
...
###################################
allLinksDF$occur <- rowSums(allLinksDF)
head(allLinksDF)
allLinksDF <- allLinksDF[order(allLinksDF$occur, decreasing = T), ]

allLinksDFTOP <-
  allLinksDF[order(allLinksDF$occur, decreasing = T)[1:500], ]
