...
#    outputPath
  ...
  file2 <- paste(outputPath,
                 paste(name, "allExons_GCDist.png", sep = "."), sep = "/")
  png(
    file2,
    width = 12,
    height = 8,
  ...
  ##############################################
  file4 <-
    paste(outputPath, paste(name, "allExons_boxplots.png", sep = "."), sep = "/")
  png(
    file4,
    width = 12,
    height = 8,
  ...
  ##################################################
  Q1Numeric<-data.frame(matrix(unlist(apply(Q1dPSI,1, as.numeric)), 
                             ncol=ncol(Q1dPSI),
                             nrow=nrow(Q1dPSI), byrow = T))
...
##################################################
Q3Numeric<-data.frame(matrix(unlist(apply(Q3dPSI,1, as.numeric)), 
                             ncol=ncol(Q3dPSI),
                             nrow=nrow(Q3dPSI), byrow = T))
...
aggDFEvents <- data.frame(aggregate(
  ...
  out_GC<-paste(outputPath,paste(name,"gene_GC.tab", sep="_"), sep="/")
  ddDF_Sorted<-ddDF[order(ddDF$GENEID),]
  head(ddDF_Sorted)
  write.table(ddDF_Sorted, file=out_GC,col.names = NA, sep="\t")
  #Agrregate by GENE
  aggDF<-data.frame(aggregate(ddDF_Sorted$GC_content, by=list(ddDF$GENEID), FUN=paste, collapse=":"))
  colnames(aggDF)<-c("gene", "gcContent")
  head(aggDF)
  
  out_GCAgg<-paste(outputPath,paste(name,"gene_Agg_GC.tab", sep="_"), sep="/")
  write.table(aggDF, file=out_GCAgg, sep="\t", col.names = NA)
