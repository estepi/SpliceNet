...
setwd(folder)
featuresClass <-
  read.table("features_class_order.csv",
             sep = "\t",
             header = T)
head(featuresClass)
...
dir(folder)
exp <-
  data.frame(dir(folder, pattern = "^exons_up_c"), stringsAsFactors = F)
colnames(exp) <- "exp"
...
Numeric <- data.frame(matrix(
  unlist(apply(meanTable, 1,
               as.numeric)),
  ncol = ncol(meanTable),
  nrow = nrow(meanTable),
  byrow = T
...
meanBgF$order <-
  combined$order[match(meanBgF$feature, combined$feature)]
meanBgF$shortName <-
  combined$shortName[match(meanBgF$feature, combined$feature)]
meanBgF$meanVal <- 0
meanBgF$MEAN <-
  round(meanBg[match(meanBgF$feature, names(meanBg))], digits = 2)
head(meanBgF)
head(combined)
