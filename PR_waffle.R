...
rankings
sampleClass <-
  read.csv("../SpliceNetData/class_colors2020.tab",
           sep = "\t",
           header = T)
head(sampleClass)
##################################################################
...
for (i in 1:nrow(rankings))
{
  ...
  print(file)
  name <-
    gsub("_CentralityRanking.tab", "", rankings$V1[i])
  print(name)
...
allNodes <-
  unique(unlist(nodesListNames))
length(allNodes)
allNodesDF <-
  data.frame(matrix(0, ncol = length(nodesList), nrow = length(allNodes)), row.names = allNodes)
head(allNodesDF)
...
forPlotMelt <-
  melt(forPlot, variable_name = "gene")
colnames(forPlotMelt)[2] <- "network"
ii <-
  match(forPlotMelt$gene, sampleClass$Gene.Symbol)
...
##################################################################
forPlotMelt$class <-
  sampleClass$Class...family.small[ii]
forPlotMelt$color <- sampleClass$color[ii]
#################################################
complex <- unique(forPlotMelt$class)
head(complex)
cc <-
  match(complex, sampleClass$Class...family.small)
color <- sampleClass$color[cc]
names(color) <- complex
write.table(forPlotMelt,
            "summary_ggplot_PR.tab",
            sep = "\t",
            col.names = NA)
