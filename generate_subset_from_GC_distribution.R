...
file1<-paste(outputPath,paste(name, "boxplots.png", sep="."), sep="/")
  png(file1,    width =12, height = 8, units = 'in', res = 300)
...
#recomputo las distribuciones:
  ...
  length(which(onlyValues$GC<=Q1L))#1215
  length(which(onlyValues$GC<=Q1L&onlyValues$GC>=LWL))#1215
  length(which(onlyValues$GC>=Q3L))#1215
  length(which(onlyValues$GC<=UWL))#1215
  length(which(onlyValues$GC>=UWL))#3
  #####################################################################
  #printing
  ...
  outLW_GC<-paste(outputPath,paste(name,"LW_GC_psi.tab", sep="_"), sep="/")
  write.table(onlyValues[onlyValues$GC<=LWL,],outLW_GC, col.names = NA, sep="\t")
  ######################################################################
  print(paste("GC Q1 value ", round(Q1L, digits = 2), sep=": "))
  print(paste("Num of Events lower than GC Q1 ", length(which(onlyValues$GC<=Q1L)), sep=": "))
  outQ1_GC<-paste(outputPath,paste(name,"Q1_GC_psi.tab", sep="_"), sep="/")
  ...
  print(paste("Num of Events greater than LW and lower than GC Q1 ", 
              length(which(onlyValues$GC<=Q1L)&
                       length(which(onlyValues$GC>=LWL)))
              , sep=": "))
  outLW_Q1_GC<-paste(outputPath,paste(name,"LW_Q1_GC_psi.tab", sep="_"), sep="/")
  write.table(onlyValues[onlyValues$GC<=Q1L&onlyValues$GC>=LWL,]
                ,outLW_Q1_GC, 
                col.names = NA, sep="\t")
  ######################################################################
  print(paste("GC Q3 value ", Q3L, sep=": "))
  print(paste("Num of Events greater than GC Q3 ", length(which(onlyValues$GC>=Q3L)), sep=": "))
  outQ3_GC<-paste(outputPath,paste(name,"Q3_GC_psi.tab", sep="_"), sep="/")
  ...
  outWD_GC<-paste(outputPath, paste(name,"UW_GC_psi.tab",sep="_"), sep="/")
  ...
  print(paste("Num of Events greater than Q3 and lower than GC UW ", 
              length(which(onlyValues$GC>=Q3L&
                             onlyValues$GC<=UWL)), 
              sep=": "))
  outQ3_UW_GC<-paste(outputPath,paste(name,"Q3_UW_GC_psi.tab", sep="_"), sep="/")
  write.table(onlyValues[onlyValues$GC>=Q3L&onlyValues$GC<=UWL,],
              outQ3_UW_GC, col.names = NA, sep="\t")
  ######################################################################
  print(paste("Files printed in", outputPath, sep=": "))
  }
###############################################################################
workingDir<-"/media/emancini/e323fb45-bfcd-44b1-9dc8-cde403de4b5a1/boots/inputGC/"
outputPath<-"/media/emancini/e323fb45-bfcd-44b1-9dc8-cde403de4b5a1/boots/inputGC/surfaces/"
inputFile<-"ES_Q3_R_psi.tab"
name="ES_Q3_R"
setwd(workingDir)
#############
table<-read.table( inputFile,sep="\t", dec = "," , header = T, row.names = 1)
colnames(table)
...
write.table(features[,c(1,19)], "exons_GC.tab", col.names = NA, sep="\t")
#########################################
...
png(file2,  width =12, height = 8, units = 'in', res = 300)
hist(table$GC, breaks=100,xlim=c(0,1) ,
