...
##################################################################
setwd("SpliceNetRes/")
class<-read.delim("-../SpliceNetData/class_colors2020.txt", header = T)
##################################################################
expected <-
  read.table(
...
neverRecovered <-
  labchiplinks[which(!labchiplinks %in% allLinksNets)]
#FIG7b
...
png(width = 800,
    height = 400,
    file = "neverRecovered_Vs_labchip.png")
...
#FIG7d
png(
  file = "NeverRecovered_upsetR.png",
  width = 12,
  height = 8,
  units = 'in',
  res = 300
...
SFsEvidenceTop <-
  SFsEvidence[order(SFsEvidence$Freq, decreasing = T)[1:20],]
#####################################################
#fig7e
png(width = 600,
    height = 400,
    file = "neverRecoveredEvidenceTop.png")
...
auxdf <- data.frame(
  ES_DS_single = all %in% linksList[[1]],
...
auxdf <- data.frame(
  CROSS_AS = all %in% newlinksCross[[1]],
...
###############################
png(
  "ES_DS_total_occur_exp.png",
  width = 4,
  height = 4,
  units = 'in',
  res = 300
)
ggplot(ES_DS_totalSort[ES_DS_totalSort$color == "exp", ], aes(Xvar, occur, color =
                                                                color)) +
  geom_point() +
  theme_bw() +
...
###############################
png(
  "ES_SS_t_occur_exp.png",
  width = 4,
  height = 4,
  units = 'in',
  res = 300
)
ggplot(ES_SS_tSort[ES_SS_tSort$color == "exp", ], aes(Xvar, occur, color =
                                                        color)) +
  geom_point() +
  theme_bw() +
...
dev.off()
###############################
ES_SS_s <- read.table("ES_SS_single.tab", row.names = 1)
...
dev.off()
###############################
ES_DS_s <- read.table("ES_DS_single.tab")
